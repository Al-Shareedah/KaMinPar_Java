...
set style line 3 lw 5 pt 5 ps 5 lc rgb "#2ca02c"   # Deep NUGP {/Symbol d}=0.0
set style line 4 lw 5 pt 13 ps 5 lc rgb "#d62728"  # Deep NUGP {/Symbol d}=57

# Add left margin for long ticks/labels

# Plot from the updated file
plot "data_pgp_ccm.txt" using 1:2 title "KaMinPar"          with linespoints ls 1, \
     "data_pgp_ccm.txt" using 1:3 title "KaHIP"             with linespoints ls 2, \
     "data_pgp_ccm.txt" using 1:4 title "ReNUP, {/Symbol d}=0.0"  with linespoints ls 3, \
     "data_pgp_ccm.txt" using 1:5 title "ReNUP, {/Symbol d}=534"   with linespoints ls 4
